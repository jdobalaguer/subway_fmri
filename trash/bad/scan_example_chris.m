...
addpath(genpath('/Users/csummerfield/data/MATLABCODE/spm8/'))
p_nii = 'data/nii/';
u_subject = [6:23];
...
for s=1:length(u_subject)
    ...
    for run=u_runs;
        data=[];
        indx=sdata.sub==u_subject(s) & sdata.sess==run;
        for f=1:length(fn);
            try
    ...
    for run = u_runs
        ...
        R = load(fullfile(p_nii,['sub' num2str(sub)],['run' num2str(run)],rpfile));
        save(['rp_run' num2str(run) '.mat'],'R')
    end
end

%% 1st level prepare the GLM
p_nii='/Users/csummerfield/data/ruz/fMRI/images/';
u_subject = 6:23;
modelname = 'rules11RL';
onsetsname = 'rules11RL_onsets';
for s=1:length(u_subject)
    ...
    statsdir =fullfile(p_nii,['sub' num2str(sub)],modelname);
    ...
    onsetsdir = [p_nii,'sub',num2str(u_subject(s)),filesep,'behavioural',filesep,onsetsname];
    
    for irun=1:length(u_runs)
        run = u_runs(irun);
        
        rundir = fullfile(p_nii,['sub' num2str(sub)],['run' num2str(run)]); % run directory
...
for s=1:length(u_subject)
    ...
    %%% main contrasts
    matlabbatch{1}.spm.stats.con.spmmat = {fullfile(p_nii,['sub' num2str(sub)],modelname,'SPM.mat')};
    ...
    save(fullfile(p_nii,['sub' num2str(sub)],modelname,'allcontrasts.mat'),'allcontrastsname')
...
modelname = 'rules11RL';
onsetsname = 'rules11RL_onsets';
groupdir = fullfile(p_nii, 'group', modelname);
mkdir(groupdir);
load( fullfile(p_nii,['sub' num2str(u_subject(1))],modelname,'allcontrasts.mat'))
...
for i=1:length(condindx)
    ...
    for s=1:length(u_subject)
        sub=u_subject(s);
        orifile = fullfile(p_nii,['sub' num2str(sub)],modelname,sprintf('spmT_%04i',indx));
        ...
        orifile = fullfile(p_nii,['sub' num2str(sub)],modelname,sprintf('con_%04i',indx));
