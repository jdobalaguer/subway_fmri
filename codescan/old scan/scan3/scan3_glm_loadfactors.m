...
function [timefacs,datafacs] = scan3_glm_loadfactors(session)
    % generate factors (modulated events synchronised with the trial sdata)
    ...
    for i_lsdata = 1:nb_lsdata
        ...
        data = load(file);
        fprintf('scan3_loadfactors: loading datafile: %s\n',file);
        scans{i_lsdata} = data;
    end
