...
function [timeregs,dataregs] = scan4_glm_loadregressors(session,verbose)
    ...
    for i_lsdata = 1:nb_lsdata
        ...
        data = load(file);
        if verbose; fprintf('scan4_glm_loadregressors: loading datafile: %s\n',file); end
        scans{i_lsdata} = data;
    end
