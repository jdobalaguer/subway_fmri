...
function scan3_mvpa(region,ii_sub)

    %% check
    % mvpa toolbox
    if ~exist('mvpa_add_paths','file')
        error('scan3_mvpa: error. requires the mvpa toolbox, if it is unavailable this will not execute');
    else
        paths     = strsplit(path(),':');
    ...
    if ~exist('newff','file')
        fprintf('scan3_mvpa: warning. requires the neural networking toolbox.\n');
    end
    ...
    fprintf('\n\n');
    fprintf('\nscan3_mvpa: ***********************************************');
    fprintf('\nscan3_mvpa: correlate HRF *********************************');
    for i_sub = ii_sub
        tools_getworkspace();
        scan3_mvpa_hrf(tmp_workspace);
        clear('tmp_workspace');
    end
