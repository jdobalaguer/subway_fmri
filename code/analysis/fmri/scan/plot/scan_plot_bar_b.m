...
function scan_plot_bar_b(varargin)
    % SCAN_PLOT_BAR_B(PATH_TO_GLM)
    % SCAN_PLOT_BAR_B(SCAN,MASK,CONTRASTS)
    ...
    if isstr(varargin{1}) && nargin==3,
        assert(exist([varargin{1},'/scan.mat'],'file')>0, 'scan_plot_bar_b: error. no "scan.mat" file found');
    ...
    elseif isstruct(varargin{1}) && nargin==1
        ...
        error('scan_plot_bar_b: error. wrong input');
    end
    
    %% ASSERT
    assert(isfield(scan.glm.plot,'mask'), 'scan_plot_bar_b: error. no mask specified');
    ...
    for i_contrast = 1:length(scan.glm.plot.contrast)
        dire_contrast = [scan.dire.glm_beta,'con_',scan.glm.plot.contrast{i_contrast},filesep()];
        file_subject = dir([dire_contrast,'beta_sub*_con*.',scan.glm.plot.extension]);
    ...
    fig_barweb(m,e,...
            [],...                                                  width
            {''},...                                                group names
            'beta comparison between contrasts',...                 title
            'contrasts',...                                         xlabel
            'beta value',...                                        ylabel
