...
%     image_compression();            % image             (compression)
        ...
        for i_sub = u_subject
            ...
            for i_run = u_run
                ...
                if ~exist(dir_spc,'dir') || isempty(dir([dir_spc,'g',prefix,'images*.nii']))
                    ...
                    run_images = spm_select('List', dir_img, ['^',prefix,'images.*\.nii$']);
        ...
        for i_sub = u_subject
            ...
            for i_run = u_run
                ...
                if ~exist(dir_rea,'dir') || isempty(dir([dir_rea,'u',prefix,'images*.nii']))
                    spm_select('clearvfiles');
                    raw_func_filenames{i_run}    = spm_select('List', dir_spc,['^',prefix,'images.*\.nii$']);
                    filenames_for_realign{i_run} = strcat(dir_spc,raw_func_filenames{i_run}); 
    ...
    function realignment_move()
        ...
        for i_sub = u_subject
            ...
            for i_run = u_run
                ...
                dir_rea = strcat(dir_run,'realignment',filesep);
                mkdirp(dir_rea);
                if ~isempty(dir([dir_spc,'u',prefix,'image*'])),    movefile([dir_spc,'u',prefix,'image*'],dir_rea);   end
                if ~isempty(dir([dir_spc,'*.txt'])),                movefile([dir_spc,'*.txt'],dir_rea); end
                if ~isempty(dir([dir_spc,'*.mat'])),                movefile([dir_spc,'*.mat'],dir_rea); end
                if ~isempty(dir([dir_spc,'mean*'])),                movefile([dir_spc,'mean*'],dir_rea); end
            end
        end
    end
        ...
        for i_sub = u_subject
            ...
            for i_run = u_run
                ...
                if ~exist(dir_nor,'dir') || isempty(dir([dir_nor,sprintf('w%d',pars_voxs),prefix_epi,'images*.nii']))
                    spm_select('clearvfiles');
                    raw_func_filenames{i_run} = spm_select('List', dir_rea, ['^',prefix_epi,'images.*\.nii$']);
                    filenames_for_normalise{end+1} = strcat(dir_rea,raw_func_filenames{i_run});
                end
            end
    ...
    function realignment_compression()
        fprintf('Realignment compression: %s\n');
    ...
    %% SMOOTH
    function smoothing()
        prefix = sprintf('w%dug',pars_voxs);
        jobs = {};
        for i_sub = u_subject
            dir_sub = strtrim(dir_subs(i_sub,:));
            dir_epi3 = strtrim(dir_epis3(i_sub,:));
            ...
            for i_run = u_run
                ...
                if ~exist(dir_smt,'dir') || isempty(dir([dir_smt,'s',prefix,'images*.nii']))
                    spm_select('clearvfiles');
                    raw_func_filenames{i_run}    = spm_select('List', dir_nor, ['^',prefix,'images.*\.nii$']);
                    filenames_for_smooth{end+1} = strcat(dir_nor,raw_func_filenames{i_run});
                end
            end
...
%% NORMALISATION: Compression
    function normalisation_compression()
        fprintf('Normalisation compression: %s\n');
        scan_zip(1,'nii3nor');
    end
end
