...
%     ii_short = (data.optm_dist_station_journey<8);
    ii_regular = logical(data.vbxi_regular_in);
    ii_change  = logical(data.resp_subline_C);
    
%     ii = ii_bool;
%     ii = ii_corr;
%     ii = ii_forw & ii_bool & ii_regular);
    ii = ii_forw & ii_bool;
%     ii = ii_forw & ii_hard;
%     ii = ii_corr & ii_hard;
...
%     optm_time_station_exchange = r_data_Dexchange;
    ...
    r_subgoal_time = nan(size(data.resp_dist_nextC));
    r_subgoal_time(~isnan(data.resp_dist_prevC)) = data.resp_dist_prevC(~isnan(data.resp_dist_prevC));
    r_subgoal_time(~isnan(data.resp_dist_nextC)) = data.resp_dist_nextC(~isnan(data.resp_dist_nextC));
    r_subgoal_time(~isnan(data.resp_dist_prevC) & ~isnan(data.resp_dist_nextC)) = nan;
    r_subgoal_time(data.resp_subline_C) = 0;

    r_ignore_time = nan(size(data.resp_dist_nextI));
    r_ignore_time(~isnan(data.resp_dist_prevI)) = data.resp_dist_prevI(~isnan(data.resp_dist_prevI));
    r_ignore_time(~isnan(data.resp_dist_nextI)) = data.resp_dist_nextI(~isnan(data.resp_dist_nextI));
    r_ignore_time(~isnan(data.resp_dist_prevI) & ~isnan(data.resp_dist_nextI)) = nan;
    r_ignore_time(data.resp_subline_I) = 0;
    
%     r_subgoal_time = nan(size(data.resp_dist_nextC));
%     r_subgoal_time(isnan(data.resp_dist_prevC)) = data.resp_dist_nextC(isnan(data.resp_dist_prevC));
%     r_subgoal_time(isnan(data.resp_dist_nextC)) = data.resp_dist_prevC(isnan(data.resp_dist_nextC));
%     r_subgoal_time(data.resp_subline_change) = 0;
%     r_subgoal_time(-data.resp_dist_prevC > data.resp_dist_nextC) = data.resp_dist_nextC(-data.resp_dist_prevC > data.resp_dist_nextC);
%     r_subgoal_time(-data.resp_dist_prevC < data.resp_dist_nextC) = data.resp_dist_prevC(-data.resp_dist_prevC < data.resp_dist_nextC);
%     r_subgoal_time(isnan(data.resp_dist_prevC) & isnan(data.resp_dist_nextC)) = -100;
    
%     r_subgoal_time = isnan(data.resp_dist_prevC);
%     r_subgoal_time = (data.vbxi_subline_goal == 1);
...
%     scan.mvpa.mask       = 'linear/change.img';
    ...
    % NEW
    scan.mvpa.mask       = 'new/C-LIR_sma.img';
%     scan.mvpa.mask       = 'new/XS_sma.img';
%     scan.mvpa.mask       = 'new/XS_precentral.img';
%     scan.mvpa.mask       = 'new/XS_thalamus.img';
%     scan.mvpa.mask       = 'new/R(Dgoal)_mpfc.img';
%     scan.mvpa.mask       = 'new/C-LIR_caudate_right.img';
%     scan.mvpa.mask       = 'new/X_frontsup.img';
    scan.mvpa.mask       = 'new/XS_precuneus.img';
    scan.mvpa.mask       = 'new/X&Dgoal_frontsup.img';
...
%                           'L_C', ... overall length of optimal subjourney', ...
                          ...
                          'Distance to goal',...
                          'Lines left to goal',...
                          'Time relative to "change"',...
                          'Time relative to "ignore"',...
                       }  ...
        },'level',   { {                                        ... level
