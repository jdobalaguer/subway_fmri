...
function [timeruns,dataruns] = scan_glm_generatefactors(datafiles)
    ...
    for i_datafiles = 1:length(datafiles)
        ...
        for i_break = 1:nb_breaks
            ...
            % data
            dataruns{end+1} = add_fields(struct_index(datafile.data,ii_break),datafile.map);
        end
    end
...
%% add fields
function data = add_fields(data,map)
    % numbers
