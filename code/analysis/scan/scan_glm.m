...
function mod_onset = get_modonset(timerun,u_mod)
    ...
    for i_mod = 2:length(mod_onsets)
        assert(length(mod_onsets{i_mod-1})==length(mod_onsets{i_mod}) ,'scan_process: get_modonset: error. different onsets');
        assert(all(  (mod_onsets{i_mod-1})==      (mod_onsets{i_mod})),'scan_process: get_modonset: error. different onsets');
    end
end
    ...
    while i_cond1 < l_cond
        ...
        while i_cond2 < l_cond
            ...
            if strcmp(cond{i_cond1}.name,cond{i_cond2}.name)
                assert(length(cond{i_cond1}.onset)==length(cond{i_cond1}.onset) ,'scan_process: merge_cond: error. different onsets');
                assert(all(  (cond{i_cond1}.onset)==      (cond{i_cond1}.onset)),'scan_process: merge_cond: error. different onsets');
