...
function imgs = scan_dprime(session)
    ...
    %% get regressors
    fprintf('\n');
    fprintf('scan_dprime: get regressors \n');
    times  = scan_glm_loadregressors(session);
    
    %% get times
    fprintf('\n');
    fprintf('scan_dprime: get times \n');
    cats   = cell(1,nb_subs);
    j_run = 0;
    for i_sub = u_sub
        ...
        cat = cell(1,nb_runs);
        for i_run = u_run
            fprintf('scan_dprime: participant %02d run %02i \n',i_sub,i_run);
            j_run = j_run + 1;
            ...
            cat{i_run} = scan_dprime_regressors(time,pars_tr,pars_bold,nb_scans);
        end
        cats{i_sub} = cat;
    end
    ...
    imgs = cell(1,nb_subs);
    for i_sub = u_sub
        fprintf('scan_dprime: participant %02d \n',i_sub);
        dir_sub  = strtrim(dir_subs(i_sub,:));
        dir_nii3 = strtrim(dir_epis3(i_sub,:));
        dir_runs = dir([strtrim(dir_epis3(i_sub,:)),'run*']); dir_runs = strcat(strvcat(dir_runs.name),'/');
        nb_runs  = size(dir_runs, 1);
