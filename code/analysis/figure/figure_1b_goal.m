...
function figure_1b()
    ...
    x = 1000 * getm_mean(data.resp_reactiontime(ii),data.expt_subject(ii),data.optm_dist_station_goal(ii),data.resp_direction_switch(ii),data.vbxi_exchange_in(ii));
    t = getm_mean(data.optm_dist_station_goal(ii),data.expt_subject(ii),data.optm_dist_station_goal(ii));
    t = meeze(t)';
    m = meeze(x);
    e = steeze(x);
