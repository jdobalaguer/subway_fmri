...
function newnii = scan_resamplenii(oldnii,newsize,interpolation)
    ...
    switch interpolation
        case 'average'
            newnii = resample_average();
        case 'linear'
            newnii = resample_linear();
        otherwise
            error('scan_resamplenii: error. interpolation "%s" no valid\n',interpolation);
    end
    
    %% check
    if(all(size(newnii)~=newsize))
        ...
        for i_x = 1:length(x)
            ...
            for i_y = 1:length(y)
                ...
                for i_z = 1:length(z)
                    ...
                    neighbourhood = oldnii([fx,cx],[fy,cy],[fz,cz]);
